...
## 用户的风险选择(2026-08-10)+ 关键修正
- 用户答:目标定 **30%**,理由 = "30% 回撤不会逼我手动干预系统"。**锚点满分**(behavioral:按"不会逼我砍仓的回撤"下注 = 给自己的手上锁,防的是量化最致命的亏损——人在最低点关系统把纸面回撤变永久损失)。
- **教了关键区别:vol 目标 ≠ 最大回撤**。实测:target 30%→MaxDD −36.1%(超红线 6pt);target 25%→−30.8%;要回撤落 30% 附近,目标≈25%。
- **再让一步:未来 > 回测**。−36/−30 都是回测最坏值,实盘常突破。守住"30% 才干预"应留缓冲 → 建议 **vol 目标 20–22%**(回测 MaxDD −25~−28%),给未知 regime 预留。用户待定最终目标。
- 纪律沉淀:**按能扛的回撤反推目标,再留缓冲**(= 专业风控)。
